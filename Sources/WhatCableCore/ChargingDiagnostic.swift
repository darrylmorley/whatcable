...
extension ChargingDiagnostic {
    public init?(
        ...
        if let cableW = cableMaxW, cableW < chargerMaxW {
            ...
            self.summary = tr("Cable is limiting charging speed")
            self.detail = tr("Charger can deliver up to \(chargerMaxW)W, but this cable is only rated to carry \(cableW)W. Replace the cable to charge faster.")
            ...
            self.summary = tr("Charging at \(n)W (charger can do up to \(chargerMaxW)W)")
            self.detail = tr("Both the charger and cable can do more, but the Mac is currently asking for less. This is normal once the battery is mostly full, or when the system is idle.")
        } else if let n = negotiatedW {
            self.bottleneck = .fine(negotiatedW: n)
            self.summary = tr("Charging well at \(n)W")
            self.detail = tr("Charger and cable are well-matched.")
        } else {
            self.bottleneck = .chargerLimit(chargerW: chargerMaxW)
            self.summary = tr("Charger advertises up to \(chargerMaxW)W")
            self.detail = tr("Negotiation hasn't completed yet.")
        }
    }
}
